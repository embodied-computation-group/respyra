\begin{table}[ht]
\centering
\caption{Session-level descriptive statistics for the single-participant validation study (4~sessions, 12~trials each). BPM = breaths per minute; Depth = peak-to-trough breathing amplitude; Cycles = total detected respiratory cycles across all trials in the session. MAE values are visual MAE (mean absolute error between target and displayed feedback; for veridical trials, visual MAE equals physical MAE). Dur.\ = session duration in minutes. Values are $M \pm SD$ across trials.}

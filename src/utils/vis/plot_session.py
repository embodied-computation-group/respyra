...
def compute_trial_stats(df: pd.DataFrame) -> pd.DataFrame:
    ...
    stats = tracking.groupby(['trial_num', 'condition']).agg(
        mae=('abs_error', 'mean'),
...
def compute_baseline_cal(df: pd.DataFrame) -> pd.DataFrame:
    ...
    cal = baseline.groupby(['trial_num', 'condition']).agg(
...
def _plot_full_trace(ax, df):
    ...
    ax.plot(valid['session_time'], valid['force_n'],
            color=FORCE_COLOR, linewidth=0.6, alpha=0.85, label='Breathing')

    # Target overlay (tracking only)
    target_data = df[(df['phase'] == 'tracking') & df['target_force'].notna()]
    if not target_data.empty:
        ax.plot(target_data['session_time'], target_data['target_force'],
                color=TARGET_COLOR, linewidth=1.0, linestyle='--',
...
def _plot_error_timeseries(ax, tracking):
    ...
    if tracking.empty:
        ax.text(0.5, 0.5, 'No tracking data', transform=ax.transAxes,
                color='#666666', ha='center', va='center', fontsize=12)
        return
...
def _plot_trial_mae_bars(ax, trial_stats):
    ...
    bars = ax.bar(range(len(trials)), mae_vals, color=colors, alpha=0.85,
    ...
    ax.text(len(trials) - 0.5, overall_mae + 0.02, f'mean={overall_mae:.2f}',
            color='white', fontsize=7, ha='right', va='bottom')

    # Legend for conditions
    unique_conds = trial_stats['condition'].unique()
    handles = [plt.Rectangle((0, 0), 1, 1, color=CONDITION_COLORS.get(c, '#999'))
        ...
        if not trial_stats.empty:
            for i, cond in enumerate(conditions):
                ...
                if not cond_stats.empty:
                    ...
                    ax.scatter(i + jitter, cond_stats['mae'],
                               color='white', s=30, zorder=5, edgecolors='none',
                               alpha=0.8, label='Trial MAE' if i == 0 else '')
...
def _plot_baseline_stability(ax, baseline_cal):
    ...
    for i, (t, c, a, col) in enumerate(zip(trials, centers, amplitudes, colors)):
        ax.errorbar(i, c, yerr=a, fmt='o', color=col, markersize=8,
                    capsize=6, capthick=1.5, elinewidth=1.5,
                    markeredgecolor='white', markeredgewidth=0.5)

    # Connect centers with a line to show drift
    ax.plot(range(len(trials)), centers, color='white', linewidth=0.8,
            alpha=0.4, linestyle='--')
    ...
    handles = [plt.Line2D([0], [0], marker='o', color=CONDITION_COLORS.get(c, '#999'),
                           linestyle='', markersize=8) for c in unique_conds]
    labels = [c.replace('_', ' ') for c in unique_conds]
    ax.legend(handles, labels, loc='upper right', fontsize=8,
              facecolor='#1a1a2e', edgecolor='#333333', labelcolor='white')
...
def _plot_summary_text(ax, df, trial_stats, baseline_cal, csv_path):
    ...
    for spine in ax.spines.values():
        spine.set_color('#333333')
    ax.set_title('Summary Statistics', color='white', fontsize=11,
    ...
    if not baseline_cal.empty:
        ...
        lines.append(f'Baseline center: {centers.mean():.2f} N '
                     f'(range {centers.min():.2f}-{centers.max():.2f})')
        lines.append(f'Baseline amplitude: {amps.mean():.2f} N '
                     f'(range {amps.min():.2f}-{amps.max():.2f})')

    text = '\n'.join(lines)
    ax.text(0.05, 0.95, text, transform=ax.transAxes, color='white',
